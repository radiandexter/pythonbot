...
from bot.constants import Channels, Colours, DEBUG_MODE, Emojis, Guild, Roles, ThreadArchiveTimes
...
if DEBUG_MODE:
    NOMINATION_THREAD_ARCHIVE_TIME = ThreadArchiveTimes.DAY.value
else:
    NOMINATION_THREAD_ARCHIVE_TIME = ThreadArchiveTimes.WEEK.value
...
class Reviewer:
    ...
    async def post_review(self, user_id: int, update_database: bool) -> None:
        ...
        thread = await last_message.create_thread(
            ...
            auto_archive_duration=NOMINATION_THREAD_ARCHIVE_TIME
        )
